...
| Git | ✅ 11 commit di branch `master`: `a796b1b` (checkpoint Fase 1+Finance+HR), `1fa8c1f` (Sales), `8ee2ef6` (update dok), `6440e11` (Purchasing), `f9e58a0` (update dok), `7590cd5` (Warehouse), `c0e600b` (Production), `b08a005` (QC), `359d6dc` (Asset), `51b6567` (Docker fix + audit topics), `cc579b1` (dok), `0d88230` (BI Dashboards), `6479137` (Forecasting+Anomaly). Company/Branch switcher belum di-commit (lihat bawah). Belum ada remote. |
...
Company/Branch switcher (lihat detail di bawah) sudah menutup satu gap besar yang tersisa dari Fase 2. Belum ada rencana Fase 3 yang terdokumentasi. Rekomendasi untuk sesi berikutnya:
1. **Tanya user dulu** apa prioritas selanjutnya — kandidat wajar: (a) polish/hardening modul yang sudah ada (validasi lebih ketat, penanganan error edge case, test otomatis), (b) branch-level filtering sungguhan di backend (lihat catatan "belum dikerjakan" di bagian switcher di bawah), (c) deployment/production-readiness (Docker image per service, CI/CD, environment config selain localhost), atau (d) modul benar-benar baru di luar rencana awal.
2. Kalau user minta lanjut modul serupa pola CRUD (seperti Warehouse/Production/QC/Asset), pola arsitektur di bawah ("Pola Arsitektur yang Sudah Established") masih berlaku persis sama.
3. Kalau user minta sesuatu yang sifatnya infrastruktur/lintas-service (seperti AI/BI kemarin), **tanya scope dulu sebelum coding** — itu pola yang terbukti berguna dua sesi terakhir.
...
- **Branch masih DISPLAY-ONLY** (keputusan user eksplisit sesi ini): TIDAK ADA backend list endpoint manapun (dari 12 service) yang menerima filter `branch_id` — semua masih filter `company_id` saja. Switcher branch cuma mengubah state UI, belum memengaruhi data yang tampil sama sekali. Juga: TIDAK ADA form create di halaman manapun yang expose input `branch_id` (dicek lewat grep, nol hasil) meski field `branch_id` ada di HAMPIR SEMUA model backend (`*string`, nullable) — jadi `branchId` dari context belum dipakai/dikirim ke mana pun. Ini sengaja tidak dipaksakan sesi ini karena: (a) 0 branch ada di seed data sampai sesi ini bikin 1 buat testing, (b) tidak ada cara verifikasi behavior "pre-fill branch_id" tanpa data branch sungguhan. Kalau mau lanjutkan: tambah `branch_id: branchId || null` ke payload create yang relevan, TAPI pertimbangkan juga apakah user mau branch benar-benar jadi filter list (bukan cuma default field) — itu perubahan lebih besar (tambah query param `branch_id` opsional ke 40+ endpoint list di 12 service).
