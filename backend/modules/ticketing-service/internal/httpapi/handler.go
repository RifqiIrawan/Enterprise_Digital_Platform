...
func (h *Handler) Register(mux *http.ServeMux) {
	...
	mux.HandleFunc("GET /categories", h.listTicketCategories)
	mux.HandleFunc("POST /categories", h.createTicketCategory)
	mux.HandleFunc("PUT /categories/{id}", h.updateTicketCategory)

	mux.HandleFunc("GET /tickets", h.listTickets)
	mux.HandleFunc("POST /tickets", h.createTicket)
	mux.HandleFunc("PUT /tickets/{id}", h.updateTicket)
	mux.HandleFunc("POST /tickets/{id}/close", h.closeTicket)
	mux.HandleFunc("POST /tickets/{id}/reopen", h.reopenTicket)

	mux.HandleFunc("GET /comments", h.listTicketComments)
	mux.HandleFunc("POST /comments", h.createTicketComment)
}

func (h *Handler) health(w http.ResponseWriter, r *http.Request) {
	writeJSON(w, http.StatusOK, map[string]string{"status": "ok", "service": "ticketing-service"})
}
