...
const accountColumns = `id, company_id, branch_id, account_code, name, industry, website, phone, email, address, account_type, owner_user_id, notes, created_at, updated_at`

func scanAccount(row pgx.Row, a *model.Account) error {
	return row.Scan(&a.ID, &a.CompanyID, &a.BranchID, &a.AccountCode, &a.Name, &a.Industry, &a.Website, &a.Phone, &a.Email, &a.Address, &a.AccountType, &a.OwnerUserID, &a.Notes, &a.CreatedAt, &a.UpdatedAt)
}
...
func (h *Handler) updateAccount(w http.ResponseWriter, r *http.Request) {
	...
	var a model.Account
	err := scanAccount(h.pool.QueryRow(r.Context(), `
		UPDATE accounts SET name = $1, industry = $2, website = $3, phone = $4, email = $5, address = $6, account_type = $7, owner_user_id = $8, notes = $9, updated_at = now()
		WHERE id = $10
		RETURNING `+accountColumns,
		req.Name, req.Industry, req.Website, req.Phone, req.Email, req.Address, req.AccountType, req.OwnerUserID, req.Notes, id,
	), &a)
