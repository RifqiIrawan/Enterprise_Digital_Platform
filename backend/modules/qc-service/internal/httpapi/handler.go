...
func (h *Handler) Register(mux *http.ServeMux) {
	...
	mux.HandleFunc("PUT /standards/{id}", h.updateStandard)

	mux.HandleFunc("GET /inspections", h.listInspections)
	mux.HandleFunc("POST /inspections", h.createInspection)
	mux.HandleFunc("GET /inspections/{id}", h.getInspection)
}

func (h *Handler) health(w http.ResponseWriter, r *http.Request) {
	writeJSON(w, http.StatusOK, map[string]string{"status": "ok", "service": "qc-service"})
}
